function t_appendGoogleMap(recid, key) {
    if ("object" == typeof google && "object" == typeof google.maps) t_handleGoogleApiReady(recid);
        ...
        eval(runfunc);
        var langPreferences = "", mapLang = "", tildaMapElement = document.querySelector("#rec" + recid + " .t-map"), tildaMapElLang = tildaMapElement.getAttribute("data-map-language");
...
function t_handleGoogleApiReady(recid) {
    ...
    Array.prototype.forEach.call(gmaps, function(gmap) {
        ...
        var mapStyleAttr = gmap.getAttribute("data-map-style"), mapColorAttr = gmap.getAttribute("data-map-color"), mapModeAttr = gmap.getAttribute("data-map-mode"), mapZoomAttr = gmap.getAttribute("data-map-zoom"), myLatLng = arMarkers.length > 0 && new google.maps.LatLng(parseFloat(arMarkers[0].lat), parseFloat(arMarkers[0].lng)), myOptions = {
        ...
        function getMapStyle(mode, style, color) {
            ...
            if (mode && color) {
                var hsl = t_map_hexToHsl(color), saturation = 2 * hsl[1] - 100, lightness = 2 * hsl[2] - 100, invert = "bw_dark" === mode;
                return [
                    {
                        featureType: "all",
                        elementType: "all",
        ...
        var map = new google.maps.Map(gmap, myOptions), bounds = new google.maps.LatLngBounds;
        ...
        function getAnchor(marker) {
            if (marker.offset) {
                var arr = marker.offset.replace(/\s/g, "").split(","), x = -1 * +arr[0], y = -1 * +arr[1];
                return new google.maps.Point(x, y);
            }
            return new google.maps.Point(0, 48);
        }
        Array.prototype.forEach.call(arMarkers, function(el) {
            var myLatLng = new google.maps.LatLng(parseFloat(el.lat), parseFloat(el.lng)), icon = {
                url: el.url,
        ...
        function t_map__attachInfoMessage(marker, descr, title, isOpen) {
            ...
            var baloonTitle, baloonDescr, baloonContent = [
                title ? "<b>" + title + "</b>" : "",
                textarea.textContent
            ].filter(Boolean).join("<br>"), infowindow = new google.maps.InfoWindow({
        ...
        google.maps.event.addDomListener(window, "resize", function() {
            var center = map.getCenter(), zoom = parseInt(mapZoomAttr, 10);
...
function t_appendYandexMap(recid, key) {
    if ("object" == typeof ymaps && "function" == typeof ymaps.Map) t_handleYandexApiReady(recid);
        ...
        eval(runfunc);
        var mapLang = "", tildaMapElement = document.querySelector("#rec" + recid + " .t-map"), tildaMapElAttr = tildaMapElement.getAttribute("data-map-language");
...
function t_handleYandexApiReady(recid) {
    ...
    Array.prototype.forEach.call(ymapsArr, function(ymap) {
        var arMarkers = window["arMapMarkers" + recid], mapZoomAttr = ymap.getAttribute("data-map-zoom");
        window.isDragMap = !window.isMobile;
        var myLatLng = arMarkers.length > 0 && [
            parseFloat(arMarkers[0].lat),
            parseFloat(arMarkers[0].lng)
        ], myOptions = {
            zoom: parseInt(mapZoomAttr, 10),
            center: myLatLng,
            scrollZoom: !1,
            ...
            FR: "Utilisez deux doigts pour d\xe9placer la carte",
        ...
        function getIconImageOffset(marker) {
            return marker.url && marker.offset ? marker.offset.replace(/\s/g, "").split(",").map(Number) : marker.url ? [
                0,
                -48
            ] : void 0;
        }
        ...
        var placemarks = arMarkers.map(function(marker) {
            ...
            var baloonTitle, baloonDescr, baloonContent = [
                ...
                textarea.textContent
            ].filter(Boolean).join("<br>"), placemark;
        ...
        var mapPathAttr = ymap.getAttribute("data-map-path"), mapPathColorAttr = ymap.getAttribute("data-map-path-color"), mapPathWeightAttr = ymap.getAttribute("data-map-path-weight");
        ...
        var mapColorAttr = ymap.getAttribute("data-map-color"), mapModeAttr = ymap.getAttribute("data-map-mode");
        if (mapModeAttr) {
            ...
            if (groundPaneElementChild && (groundPaneElementChild.style.filter = "bw_light" === mapModeAttr ? "grayscale(1)" : "grayscale(1) invert(1)"), mapColorAttr) {
                var groundPaneElement = ymap.querySelector('ymaps[class$="ground-pane" i]'), coloredDiv = document.createElement("div");
...
function t_map_hexToHsl(H) {
    ...
    return h = 0 === delta ? 0 : cmax === r ? (g - b) / delta % 6 : cmax === g ? (b - r) / delta + 2 : (r - g) / delta + 4, (h = Math.round(60 * h)) < 0 && (h += 360), l = (cmax + cmin) / 2, [
        h,
        s = +(100 * (s = 0 === delta ? 0 : delta / (1 - Math.abs(2 * l - 1)))).toFixed(1),
        l = +(100 * l).toFixed(1)
    ];
}

//# sourceMappingURL=page69100279.e8ee64f3.js.map
